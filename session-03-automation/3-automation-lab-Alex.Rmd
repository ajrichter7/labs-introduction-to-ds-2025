...
summarize_bills <- function(df) {
  bills <- df %>% summarize(count = n(), mean_bill_length = mean(bill_length_mm, na.rm=TRUE), max_bill_length = max(bill_length_mm, na.rm=TRUE)) #You do not need to do df$bill_length_mm as the pipe function then moves the df when in a pipeline you do not need to specify the data frame again, but you can. 
  return(bills)
...
```{r, echo= FALSE, eval= FALSE}
penguins_nested <- penguins_nested %>% mutate(bill_stats = map(data, summarize_bills))
#Use map over sapply because it has a consistent output 
...
```{r, echo= FALSE, eval= FALSE}
penguins_summary <- penguins_nested %>% select(species, bill_stats) %>% unnest()
# For unnest you can specify but don't need to 
...
```{r, echo= FALSE, eval= FALSE}
final_summary <- penguins %>%  group_by(species) %>% nest() %>% mutate(bill_stats = map(data, summarize_bills)) %>% select(species, bill_stats) %>% unnest()
