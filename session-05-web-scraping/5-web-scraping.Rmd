...
# YOUR ANSWER HERE
parsed_url |> 
  rvest::html_element(xpath = '/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/p[8]') |> 
  rvest::html_text()
...
```{r}
#a
number_of_streams <- min(spotify_table$streams_billions)
#b

#individual_names <- unique(unlist(strsplit(spotify_table$artist_s, " and ")))
names <- spotify_table %>% 
  separate_rows(artist_s, , sep = "\\s*(and|featuring|&|with)\\s*") %>%
  group_by(artist_s) %>%
  summarise(songs = n(),
