...
no_tailnum <- dplyr::anti_join(
  flights,
  planes, ## Not renaming "year" to "year_built" this time
  by = "tailnum"
) 

flights %>% dplyr::anti_join(planes, by ="tailnum") %>% dplyr::count()
...
dplyr::copy_to(
  ...
  name = "airports", # we create the "planes" table
  temporary = FALSE, 
  indexes = "faa"
)
...
dplyr::left_join(
  flights_db,
  airlines_db, # taking care of name ambiguity
  by = "carrier" ## Important: Be specific about the joining column
) |>
  dplyr::filter(day == "1" && month =="1") |>
  dplyr::select(month, day, name, carrier, flight) 
